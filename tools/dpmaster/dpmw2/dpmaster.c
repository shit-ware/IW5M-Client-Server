...
static const cmdlineopt_t cmdline_options [] =
{
	...
	},
	{
		"info-file",
		"<file_path>",
		"Use <file_path> as the info file (default: " DEFAULT_INFO_FILE ")",
		{ 0, 0 },
		'\0',
		1,
		1
...
static cmdline_status_t Cmdline_Option (const cmdlineopt_t* opt, const char** params, unsigned int nb_params)
{
	...
	// Info file
	else if (strcmp (opt_name, "info-file") == 0)
	{
		if (! Com_SetInfoFilePath (params[0]))
			return CMDLINE_STATUS_INVALID_OPT_PARAMS;
	}
...
int main (int argc, const char* argv [])
{
	...
	for (;;)
	{
			 ...
			 sock_ind++)
		{
			...
			if(strstr(packet, "nprint") || strstr(packet, "print") ||
				strstr(packet, "error")) // simple fix by bluurj
			{
				Com_Printf (MSG_WARNING,
							"> WARNING: rejected packet from %s (nprint/print/error not allowed)\n",
							peer_address);
				continue;
			}
