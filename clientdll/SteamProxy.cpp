...
void SteamProxy_DoThatTwinklyStuff()
{
	if (!g_isDedicated)
	{
		if (GetFileAttributes("main") != INVALID_FILE_ATTRIBUTES)
		{
		...
		{
			Com_Error(1, "An error occurred during Steam verification. Please try again later. If this issue persists, ask on http://repziw4.de/.");
		}
