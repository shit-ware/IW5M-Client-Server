...
{
	return hasLicense ? (const char*)licenseFile : "license.dat";
}
...
void DetermineGameFlags()
{
	...
	//MessageBox(0, s.c_str(), "DEBUG", MB_OK);
	...
	int dw_licensefile = NULL;
	for (std::sregex_iterator i = words_begin; i != words_end; ++i) {
		std::smatch match = *i;
		std::string match_str = match.str();
		
		if (match_str == "+dw_licensefile") {
			i++;
			std::smatch match = *i;
			std::string match_str = match.str();
			strcpy(licenseFile, match_str.c_str());
			//MessageBox(0, match_str.c_str(), "DEBUG", MB_OK);
		}
