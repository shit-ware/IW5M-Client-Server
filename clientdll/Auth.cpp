...
#define AUTHSERVER_HOST "server.repziw4.de"
...
bool Auth_DisplayLoginDialogDownLevel(const wchar_t** usernameP, const wchar_t** passwordP, PCREDUI_INFOW info)
{
	...
	HRESULT result = CredUIPromptForCredentialsW(info, L"www.repziw4.de", NULL, NULL, username, CREDUI_MAX_USERNAME_LENGTH, password, CREDUI_MAX_PASSWORD_LENGTH, &save, CREDUI_FLAGS_GENERIC_CREDENTIALS | CREDUI_FLAGS_ALWAYS_SHOW_UI | CREDUI_FLAGS_DO_NOT_PERSIST | CREDUI_FLAGS_EXCLUDE_CERTIFICATES);
...
bool Auth_DisplayLoginDialog(const wchar_t** usernameP, const wchar_t** passwordP)
{
	...
	if (!doneFile && (err = fopen_s(&iw4mini, "iw5m.ini","r")) == 0){
	...
	info.pszCaptionText = L"Authentication for IW5M";
	info.pszMessageText = L"Please log in to use IW5M. Use your www.repziw4.de forum credentials.";
	...
	if (result == ERROR_SUCCESS)
	{
		...
		*passwordP = password;
		...
		if(save && (err = fopen_s(&iw4mini, "iw5m.ini","w")) == 0){
...
bool Auth_PerformSessionLogin(const char* username, const char* password)
{
	...
	if (curl)
	{
		...
		char url[255];
		#ifdef DEVMODE
			_snprintf(url, sizeof(url), "http://%s/devauthenticate", AUTHSERVER_HOST);
		#else
			_snprintf(url, sizeof(url), "http://%s/authenticate", AUTHSERVER_HOST);
		#endif

		char buf[8192] = {0};
		char postBuf[8192];
		_snprintf(postBuf, sizeof(postBuf), "%s&&%s&&%s", username, password, hwid);
		...
		{
			Auth_Error(va("Could not reach the www.repziw4.de server. Error code from CURL: %x.", code));
		}
...
size_t UDataReceived(void *ptr, size_t size, size_t nmemb, void *data)
{
	...
	{
		char* message;
		sprintf(message, "This version (%d) has expired. Delete cachesSxS.xml and run Launcher.exe to obtain a new version (%d).\n", BUILDNUMBER, version);
		...
		if (result == IDYES)
		{
			...
			ShellExecute(NULL, "open", "Launcher.exe", 0, 0, SW_SHOWNORMAL);
			ExitProcess(1);
		}
		else if (result == IDNO)
		{
...
void DisableOldVersions()
{
	...
	{
		char url[255];
		_snprintf(url, sizeof(url), "http://www.repziw4.net/content/mw3/version.txt");
